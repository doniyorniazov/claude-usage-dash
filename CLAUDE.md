...
- **Keychain secret reads prompt the user.** The app is ad-hoc signed, so macOS shows a keychain password dialog on every `SecItemCopyMatching` that returns secret data. Always go through `CredentialsCache.current()` (which polls the item's prompt-free modification date and re-reads the secret only when Claude Code rewrote it) — never call `Keychain.loadClaudeCredentials()` on a timer.
